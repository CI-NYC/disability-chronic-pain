---
title: "superefficient estimator primary outcome results"
...
psi_oud_hillary <- read_rds(here::here("projects/create_cohort/output/psis/psi_oud_hillary_supereff.rds"))
psi_oud <- read_rds(here::here("projects/create_cohort/output/psis/psi_oud_supereff.rds"))
 psi_oud_poison <- read_rds(here::here("projects/create_cohort/output/psis/psi_oud_poison_supereff.rds"))
...
ggsave(here::here("projects/create_cohort/figures/fig3_supereff.pdf"), height=7.5, width=7.5)

```

```{r}

contrasts_rr <- function(outcome_name, shift_name, ref_name){
    shift_df <- all_res |>
        filter(outcome == outcome_name, shift == shift_name)
    ref_df <-  all_res |>
        filter(outcome == outcome_name & shift == ref_name)
    log_eif <- (unlist(shift_df$ic) / shift_df$est) - (unlist(ref_df$ic) / ref_df$est)
    contrast_df <- tibble(
        outcome = outcome_name,
        shift = shift_name,
        ref = ref_name,
        theta = shift_df$fest / ref_df$est,
        se = sqrt(var(log_eif)/length(log_eif)),
        low = exp(log(theta) - qnorm(0.975) * se),
        high =  exp(log(theta) + qnorm(0.975) * se),
        p.value = pnorm(abs(theta)/se, lower.tail = FALSE) * 2
        )
    return(contrast_df)
}

contrast_rr_tbl <- map2_dfr(outcome_list, contrast_list, ~contrasts_rr(.x, .y[[1]], .y[[2]]))
...
t1 <- contrast_rr_tbl |>
    ...
    tab_header("Incident RR compared to neither",
               subtitle ="Primary exposure (6 months CP)")
...
t2 <- contrast_rr_tbl |>
    ...
    tab_header("Disability and Chronic Pain RR compared to PD/CP alone groups",
               subtitle ="Primary exposure (6 months CP)")
