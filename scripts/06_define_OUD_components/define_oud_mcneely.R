################################################################################
################################################################################
###  Define OUD Abuse (or poison) using McNeely's codes
###  note, we may need to split this up later
################################################################################
################################################################################
...
all <- 1:32
td <- "data/oud_info/tmp_splits_oud_mcneely_dt/"
...
foreach(i = still) %dopar%
    {print(paste(i, Sys.time()))
        ...
        oth_all_vars <-
            ...
            mutate(oud_mcneely_dt = case_when(DGNS_CD_1 %in% mcneely_codes_abuse ~ SRVC_BGN_DT,
                                            DGNS_CD_2 %in% mcneely_codes_abuse ~ SRVC_BGN_DT)) |>
            drop_na(oud_mcneely_dt) |> # drop anyone who doesn't have a date for the oud var of interest
            distinct(BENE_ID, oud_mcneely_dt)
        ...
        iph_all_vars <-
            ...
            mutate(oud_mcneely_dt = case_when(cd %in% mcneely_codes_abuse ~ SRVC_BGN_DT)) |>  # define oud variable of interest via codes
            drop_na(oud_mcneely_dt) |> # drop anyone who doesn't have a date for the oud var of interest
            distinct(BENE_ID, oud_mcneely_dt)
        
        all <- bind_rows(oth_all_vars, iph_all_vars) # stack the columns (BENE_ID and oud var of interest) on top of each other
        
        write_parquet(all, paste0("data/oud_info/tmp_splits_oud_mcneely_dt/", i, ".parquet")) # save the split
...
min_oud_mcneely_dt <- 
    ...
    filter(oud_mcneely_dt >= washout_start_dt) |> # only keep oud mcneely diagnosis codes that occur during washout period
    select(BENE_ID, oud_mcneely_dt) |>
    arrange(oud_mcneely_dt) |>
    distinct(BENE_ID, .keep_all = T) # only keep first date after washout (already arranged by date)


oud_mcneely_clean <- 
    dts_cohorts |>
    left_join(min_oud_mcneely_dt) |>
    mutate(oud_mcneely_washout_cal = case_when(oud_mcneely_dt %within% interval(washout_start_dt, washout_cal_end_dt) ~ 1,
                                                TRUE ~ 0),
           oud_mcneely_washout_cont = case_when(oud_mcneely_dt %within% interval(washout_start_dt, washout_cont_end_dt) ~ 1,
                                                 TRUE ~ 0),
           oud_mcneely_study_cal = case_when(oud_mcneely_dt %within% interval(washout_start_dt, study_cal_end_dt) ~ 1,
                                              is.na(study_cal_end_dt) & oud_mcneely_dt > washout_start_dt ~ 1,
                                              TRUE ~ 0),
           oud_mcneely_study_cont = case_when(oud_mcneely_dt %within% interval(washout_start_dt, study_cont_end_dt) ~ 1,
                                               is.na(study_cont_end_dt) & oud_mcneely_dt > washout_start_dt ~ 1,
                                               TRUE ~ 0)) |>
    select(BENE_ID, oud_mcneely_dt, 
           oud_mcneely_washout_cal, oud_mcneely_washout_cont,
           oud_mcneely_study_cal, oud_mcneely_study_cont)

write_rds(oud_mcneely_clean, "data/final/oud_mcneely.rds")
