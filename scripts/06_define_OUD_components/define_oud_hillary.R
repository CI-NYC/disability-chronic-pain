################################################################################
################################################################################
###  Define OUD hillary using Cochran's codes
################################################################################
################################################################################
...
all <- 1:32
td <- "data/oud_info/tmp_splits_oud_hillary_dt/"
...
foreach(i = still) %dopar%
    {print(paste(i, Sys.time()))
        ...
        write_parquet(all, paste0("data/oud_info/tmp_splits_oud_hillary_dt/", i, ".parquet")) # save the split
...
min_oud_hillary_dt <- 
    ...
    filter(oud_hillary_dt >= washout_start_dt) |> # only keep oud hillary diagnosis codes that occur during washout period
    select(BENE_ID, oud_hillary_dt) |>
...
oud_hillary_clean <- 
    dts_cohorts |>
    left_join(min_oud_hillary_dt) |>
    mutate(oud_hillary_washout_cal = case_when(oud_hillary_dt %within% interval(washout_start_dt, washout_cal_end_dt) ~ 1,
                                                TRUE ~ 0),
           oud_hillary_washout_12mos_cal = case_when(oud_hillary_dt %within% interval(washout_start_dt, washout_12mos_end_dt) ~ 1,
                                               TRUE ~ 0),
           oud_hillary_washout_cont = case_when(oud_hillary_dt %within% interval(washout_start_dt, washout_cont_end_dt) ~ 1,
                                                 TRUE ~ 0),
           oud_hillary_study_cal = case_when(oud_hillary_dt %within% interval(washout_start_dt, study_cal_end_dt) ~ 1,
                                              is.na(study_cal_end_dt) & oud_hillary_dt > washout_start_dt ~ 1,
                                              TRUE ~ 0),
           oud_hillary_study_cont = case_when(oud_hillary_dt %within% interval(washout_start_dt, study_cont_end_dt) ~ 1,
                                               is.na(study_cont_end_dt) & oud_hillary_dt > washout_start_dt ~ 1,
                                               TRUE ~ 0)) |>
    select(BENE_ID, oud_hillary_dt, 
           oud_hillary_washout_cal, oud_hillary_washout_12mos_cal,
           oud_hillary_washout_cont,
           oud_hillary_study_cal, oud_hillary_study_cont)

write_rds(oud_hillary_clean, "data/final/oud_hillary.rds")
