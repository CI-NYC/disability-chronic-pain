...
codes <- read_rds("projects/create_cohort/input/NDC_codes/mediation_unsafe_pain_mgmt_opioid_pain_ndc.rds")# these use the RXNORM method NDC values
...
all_codes <-
    ...
    rename(opioids_pain_rxnorm_dt = min_code_dt) |>
    mutate(opioids_pain_rxnorm_washout_cal = case_when(opioids_pain_rxnorm_dt %within% interval(washout_start_dt, washout_cal_end_dt) ~ 1,
                                                 TRUE ~ 0),
           opioids_pain_rxnorm_washout_12mos_cal = case_when(opioids_pain_rxnorm_dt %within% interval(washout_start_dt, washout_12mos_end_dt) ~ 1,
                                                  TRUE ~ 0),
           opioids_pain_rxnorm_washout_cont = case_when(opioids_pain_rxnorm_dt %within% interval(washout_start_dt, washout_cont_end_dt) ~ 1,
                                                  TRUE ~ 0),
           opioids_pain_rxnorm_study_cal = case_when(opioids_pain_rxnorm_dt %within% interval(washout_start_dt, study_cal_end_dt) ~ 1,
                                               TRUE ~ 0),
           opioids_pain_rxnorm_study_cont = case_when(opioids_pain_rxnorm_dt %within% interval(washout_start_dt, study_cont_end_dt) ~ 1,
                                                TRUE ~ 0)) |>
    select(BENE_ID,opioids_pain_rxnorm_dt, opioids_pain_rxnorm_washout_cal, opioids_pain_rxnorm_washout_12mos_cal, 
           opioids_pain_rxnorm_washout_cont, opioids_pain_rxnorm_study_cal,  opioids_pain_rxnorm_study_cont)

saveRDS(all_codes, "projects/create_cohort/data/final/opioids_pain_rxnorm.rds")
