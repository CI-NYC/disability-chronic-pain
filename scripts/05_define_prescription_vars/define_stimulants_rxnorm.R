...
codes <- read_rds("projects/create_cohort/input/NDC_codes/mediation_unsafe_pain_mgmt_stimulant_ndc.rds") # these use the RXNORM method NDC values
...
all_codes <-
    ...
    rename(stimulants_rxnorm_dt = min_code_dt) |>
    mutate(stimulants_rxnorm_washout_cal = case_when(stimulants_rxnorm_dt %within% interval(washout_start_dt, washout_cal_end_dt) ~ 1,
                                                 TRUE ~ 0),
           stimulants_rxnorm_washout_12mos_cal = case_when(stimulants_rxnorm_dt %within% interval(washout_start_dt, washout_12mos_end_dt) ~ 1,
                                                  TRUE ~ 0),
           stimulants_rxnorm_washout_cont = case_when(stimulants_rxnorm_dt %within% interval(washout_start_dt, washout_cont_end_dt) ~ 1,
                                                  TRUE ~ 0),
           stimulants_rxnorm_study_cal = case_when(stimulants_rxnorm_dt %within% interval(washout_start_dt, study_cal_end_dt) ~ 1,
                                               TRUE ~ 0),
           stimulants_rxnorm_study_cont = case_when(stimulants_rxnorm_dt %within% interval(washout_start_dt, study_cont_end_dt) ~ 1,
                                                TRUE ~ 0)) |>
    select(BENE_ID,stimulants_rxnorm_dt, stimulants_rxnorm_washout_cal, stimulants_rxnorm_washout_12mos_cal, 
           stimulants_rxnorm_washout_cont, stimulants_rxnorm_study_cal,  stimulants_rxnorm_study_cont)

saveRDS(all_codes, "projects/create_cohort/data/final/stimulants_rxnorm.rds")
