...
anxiety_icds <- read_csv("input/ICD_codes/anxiety_icd10_20230323.csv", col_names = F) |>
...
all <- 1:17
td <- "data/tafoth/tmp_splits_anxiety/" 
...
foreach(i = still) %dopar%
    {
        ...
        write_parquet(all_dg, paste0("data/tafoth/tmp_splits_anxiety/raw_", i, ".parquet"))
...
foreach(i = 1:17) %dopar%
    {
        print(paste(i, Sys.time()))
        all_dg <- read_parquet(paste0("data/tafoth/tmp_splits_anxiety/raw_", i, ".parquet")) |> collect()
        all_dg_clean <-
            all_dg |>
            ...
            ungroup()
        write_parquet(all_dg_clean, paste0("data/tafoth/tmp_splits_anxiety/", i, ".parquet"))
    }
...
write_parquet(iph_dg, "data/tafiph/anxiety_iph.parquet") # save data set
...
foreach(i = 1:17) %dopar%
    {
        oth_tmp <- open_dataset(paste0("data/tafoth/tmp_splits_anxiety/", paste0(i,".parquet")), format="parquet") |>
            collect()
        iph_tmp <- open_dataset(paste0("data/tafiph/anxiety_iph.parquet")) |>
            collect()
        clean <- 
            oth_tmp |>
            left_join(iph_tmp) |>
        ...
        write_parquet(clean, paste0("data/tafoth/tmp_splits_anxiety/", i, "_clean.parquet"))
    }
...
all_anxiety_clean <- 
    ...
    mutate(anxiety_washout_cal = case_when(anxiety_dt %within% interval(washout_start_dt, washout_cal_end_dt) ~ 1,
                                        TRUE ~ 0),
           anxiety_washout_12mos_cal = case_when(anxiety_dt %within% interval(washout_start_dt, washout_12mos_end_dt) ~ 1,
                                           TRUE ~ 0),
           anxiety_washout_cont = case_when(anxiety_dt %within% interval(washout_start_dt, washout_cont_end_dt) ~ 1,
                                         TRUE ~ 0),
           anxiety_study_cal = case_when(anxiety_dt %within% interval(washout_start_dt, study_cal_end_dt) ~ 1,
                                      TRUE ~ 0),
           anxiety_study_cont = case_when(anxiety_dt %within% interval(washout_start_dt, study_cont_end_dt) ~ 1,
                                       TRUE ~ 0)) |>
    select(BENE_ID, anxiety_dt, 
           anxiety_washout_cal,  anxiety_washout_12mos_cal, anxiety_washout_cont,
           anxiety_study_cal, anxiety_study_cont)

write_rds(all_anxiety_clean, "data/final/anxiety.rds") # save final data file
