...
bipolar_icds <- read_csv("input/ICD_codes/bipolar_icd10_20230323.csv", col_names = F) |>
...
all <- 1:17
td <- "data/tafoth/tmp_splits_bipolar/" 
...
foreach(i = still) %dopar%
    {
        ...
        write_parquet(all_dg, paste0("data/tafoth/tmp_splits_bipolar/raw_", i, ".parquet"))
...
foreach(i = 1:17) %dopar%
    {
        print(paste(i, Sys.time()))
        all_dg <- read_parquet(paste0("data/tafoth/tmp_splits_bipolar/raw_", i, ".parquet")) |> collect()
        all_dg_clean <-
            all_dg |>
            ...
            ungroup()
        write_parquet(all_dg_clean, paste0("data/tafoth/tmp_splits_bipolar/", i, ".parquet"))
    }
...
write_parquet(iph_dg, "data/tafiph/bipolar_iph.parquet") # save data set
...
foreach(i = 1:17) %dopar%
    {
        oth_tmp <- open_dataset(paste0("data/tafoth/tmp_splits_bipolar/", paste0(i,".parquet")), format="parquet") |>
            collect()
        iph_tmp <- open_dataset(paste0("data/tafiph/bipolar_iph.parquet")) |>
            collect()
        clean <- 
            oth_tmp |>
            left_join(iph_tmp) |>
        ...
        write_parquet(clean, paste0("data/tafoth/tmp_splits_bipolar/", i, "_clean.parquet"))
    }
...
all_bipolar_clean <- 
    ...
    mutate(bipolar_washout_cal = case_when(bipolar_dt %within% interval(washout_start_dt, washout_cal_end_dt) ~ 1,
                                           TRUE ~ 0),
           bipolar_washout_12mos_cal = case_when(bipolar_dt %within% interval(washout_start_dt, washout_12mos_end_dt) ~ 1,
                                           TRUE ~ 0),
           bipolar_washout_cont = case_when(bipolar_dt %within% interval(washout_start_dt, washout_cont_end_dt) ~ 1,
                                            TRUE ~ 0),
           bipolar_study_cal = case_when(bipolar_dt %within% interval(washout_start_dt, study_cal_end_dt) ~ 1,
                                         TRUE ~ 0),
           bipolar_study_cont = case_when(bipolar_dt %within% interval(washout_start_dt, study_cont_end_dt) ~ 1,
                                          TRUE ~ 0)) |>
    select(BENE_ID, bipolar_dt, 
           bipolar_washout_cal, bipolar_washout_12mos_cal,
           bipolar_washout_cont,
           bipolar_study_cal, bipolar_study_cont)

write_rds(all_bipolar_clean, "data/final/bipolar.rds") # save final data file
