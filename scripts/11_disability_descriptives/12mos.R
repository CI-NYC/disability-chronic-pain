################################################################################
################################################################################
###  DESCRIBE DISABILITY CODES (FIRST 12 months STUDY DURATION)
...
dis_cohort <-
    ...
    select(BENE_ID, washout_start_dt, washout_12mos_end_dt)
...
write_rds(dgcd_long_12mos, "data/tmp/dgcd_long_12mos.rds")

dgcd_long_12mos <-  read_rds("data/tmp/dgcd_long_12mos.rds")

dgcd_explain <- icd::explain_table(dgcd_long_12mos$code) |>
...
ids_no_code <- dis_cohort |>
    filter(!(BENE_ID %in% dgcd_long_12mos$BENE_ID)) 
...
circ_ids <- dgcd_explain |>
 ...
 map(ids, ~nrow(.x)/nrow(ok_cd_ids)*100)
